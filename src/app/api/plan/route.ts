...
import { planAgent, type PlanAgentRequest, type PlanAgentResult } from "@/server/planner";
  ...
  return async function post(request: Request): Promise<Response> {
    ...
    try {
      ...
      return Response.json(result, {
        status: result.status === "connection_required" ? 503 : result.status === "invalid_spec" ? 422 : 200,
      });
    } catch (error) {
